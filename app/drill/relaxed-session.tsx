  ...
  return <div className="drill-page">
    ...
    <main className="drill-session-main">
      {finished ? <section className="drill-result">
        ...
        {recent.length > 0 && <div className="answer-review"><h2>Pembahasan {recent.length} soal terakhir</h2>{recent.map(({question: q, answer}, i) => <article key={q.id}><span className={answer === q.correct ? 'review-correct' : 'review-wrong'}>{answer === q.correct ? 'Benar' : 'Perlu latihan'} · Soal {answered - recent.length + i + 1}</span><h3>{q.text}</h3><p>Jawabanmu: {q.answers[answer]}</p><p><strong>Jawaban benar: {q.answers[q.correct]}</strong></p><p>{q.explanation}</p></article>)}</div>}
      </section> : <>
        ...
        <div className="session-columns"><section className="session-question-card">
          {question ? <><div className="session-progress-label"><strong>Soal {checked ? answered : answered + 1}</strong><span>{answered} dikerjakan · {score} benar</span></div>
            ...
            <div className="answers" role="group" aria-labelledby="relaxed-question">{question.answers.map((answer, i) => <button key={i} disabled={checked} aria-pressed={selected === i} className={`answer ${selected === i ? 'chosen' : ''} ${checked && i === question.correct ? 'correct' : ''} ${checked && selected === i && i !== question.correct ? 'incorrect' : ''}`} onClick={() => setSession(current => ({...current, selected: i}))}><b>{String.fromCharCode(65 + i)}</b><span>{answer}</span>{checked && i === question.correct && <Check size={18}/>}</button>)}</div>
